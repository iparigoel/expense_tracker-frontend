        ...
        <div className="hidden md:block w-[40vw] h-screen bg-violet-50 bg-auth-bg-img bg-cover bg-no-repeat bg-center overflow-hidden p-8 relative">
            ...
            <div className="grid grid-cols-1 z-20">
                <StatsInfoCard icon={<LuTrendingUpDown />}
                    labels="Track Your Income & Expenses"
                    value="430,000"
                    color="bg-primary"
                />
            </div>
            <img src={img} className="w-64 lg:w-[90%] rounded-[20px] absolute bottom-10 shadow-lg shadow-blue-400/15" />
        </div>
    </div>
  );
}
...
const StatsInfoCard = ({icon, labels, value, color}) => {
    return <div className="flex gap-6 bg-white p-4 rounded-xl shadow-purple-400/10 border border-gray-200/50 z-10">
        ...
        <div>
            <h6 className="text-xs text-gray-500 -mb-1">{labels}</h6>
